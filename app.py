...
def main_menu_text(with_greeting: bool = False) -> str:
    prefix = "Здравствуйте! Я чат-бот Аралтыш.\n\n" if with_greeting else ""
    return (
        prefix
...
async def handle_callback(chat_id: str, user_id: str, payload: str, callback_id: str | None = None) -> None:
    ...
    if payload == "about:project":
        await send_message(
            chat_id,
            'Чат-бот разработан по инициативе Молодежного парламента Республики Марий Эл, при поддержке Государственного Собрания Республики Марий Эл, Партийных проектов:\n'
            '- "Цифровая Россия",\n'
            '- "Крепкая семья",\n'
            '- "Женское движение"',
            [callback_button("Главное меню", "main:menu")],
